# run_fwi_nondispersive_eps_s.jl — Non-dispersive FWI using ε_s as sole permittivity
#
# Third comparison case for Section III-D: non-dispersive forward model using
# ε_s = ε∞ + Δε (static permittivity) as the sole permittivity. This preserves
# the DC/low-frequency velocity match while removing Debye dispersion, isolating
# the dispersion-only effect.
#
# Three-way comparison:
#   (a) Dispersive FWI (baseline) — run_fwi_large_domain.jl
#   (b) Non-dispersive with ε_s (this script) — isolates dispersion effect
#   (c) Non-dispersive with ε∞ (run_fwi_nondispersive.jl) — compound effect
#
# Outputs to paper/data/:
#   fwi_nondispersive_eps_s_convergence.csv
#   fwi_nondispersive_eps_s_reconstruction_1d.csv
...
# (Same data as all other comparisons — ground truth is dispersive)
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
# Initial model: NON-DISPERSIVE with ε_s = ε∞ + Δε as sole permittivity
# Key: preserves DC/low-frequency velocity while removing dispersion
# ══════════════════════════════════════════════════════════════════════
eps_inf_init = ones(nx, ny)     # will hold ε_s values
deps_init    = zeros(nx, ny)    # NON-DISPERSIVE: deps=0
tau_init     = zeros(nx, ny)    # NON-DISPERSIVE: tau=0
sigma_init   = zeros(nx, ny)

# Layer 1: ε_s = ε∞ + Δε = 4 + 4 = 8
for j in layer1_top:min(layer1_bot, ny), i in 1:nx
    eps_inf_init[i, j] = 8.0    # ε_s for Layer 1
    sigma_init[i, j]   = 0.005
end

# Layer 2: ε_s = ε∞ + Δε = 6 + 12 = 18
for j in layer2_top:ny, i in 1:nx
    eps_inf_init[i, j] = 18.0   # ε_s for Layer 2
    sigma_init[i, j]   = 0.02
end

println("\nNON-DISPERSIVE forward model with ε_s: deps=0, tau=0")
println("  Layer 1: ε_s = 8.0 (vs ε∞ = 4.0)")
println("  Layer 2: ε_s = 18.0 (vs ε∞ = 6.0)")
println("  Air: ε = 1.0")
flush(stdout)
...
# ══════════════════════════════════════════════════════════════════════
# Run multi-source FWI with NON-DISPERSIVE (ε_s) forward model
# ══════════════════════════════════════════════════════════════════════
println("\n" * "=" ^ 60)
println("Running NON-DISPERSIVE (ε_s) multi-source FWI ($nsrc sources, $n_params params)")
println("=" ^ 60)
flush(stdout)
...
result = run_fwi_multisource(configs, obs_datas, src_waveforms,
                              ...
                              lambda=1.0)
t_fwi = time() - t_fwi
@printf("\nNon-dispersive (ε_s) FWI completed in %.1f s (%d iterations)\n", t_fwi, result.n_iter)
...
conv_file = joinpath(datadir, "fwi_nondispersive_eps_s_convergence.csv")
open(conv_file, "w") do io
    write(io, "# Non-dispersive (eps_s) FWI convergence: $nsrc sources, $n_params params, AD, seed=42\n")
...
# 2. 1D reconstruction slice
recon1d_file = joinpath(datadir, "fwi_nondispersive_eps_s_reconstruction_1d.csv")
open(recon1d_file, "w") do io
    write(io, "# 1D reconstruction (non-dispersive eps_s FWI): vertical slice at pipe center\n")
...
@printf("\nNon-dispersive (ε_s) reconstruction quality:\n")
...
println("\n=== Non-dispersive (ε_s) FWI comparison complete ===")
